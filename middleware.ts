...
export async function middleware(request: NextRequest) {
  try {
    ...
    // If supabase client creation failed, just continue without auth
    if (!supabase) {
      console.log("Middleware: Supabase client not available, skipping auth checks");
      return response;
    }
    ...
    // If there's any error in middleware, just continue without auth
